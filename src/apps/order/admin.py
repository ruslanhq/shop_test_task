...
class OrderAdmin(admin.ModelAdmin):
    ...
    def confirm_order(self, request, object_id, *args, **kwargs):
        ...
        if payment and payment.status == PaymentStatus.PAID and order.status != OrderStatus.CONFIRMED:
            ...
            messages.error(request, "Платеж не имеет статус 'Оплачен' или заказ уже подтвержден.")
            return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
    ...
    def render_change_form(self, request, context, *args, **kwargs):
        ...
        if obj and payment and payment.status == PaymentStatus.PAID and obj.status != OrderStatus.CONFIRMED:
